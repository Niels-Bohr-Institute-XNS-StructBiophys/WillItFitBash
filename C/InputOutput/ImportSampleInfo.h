...
int ImportSampleInformation(struct Dataset * Data, double * VolumesOfMolecules, char SamplesFileLocation[256], int NumberOfSampleInformations, int NumberOfSpectra, char *ModificationName)
{
	...
	char Dummy[2 + 6 * (NumberOfSampleInformations + 2)][256] ;
	...
	printf("%s\n",Dummy[11 + 4 * NumberOfSampleInformations]) ;
	// memset( ModificationName, '\0', sizeof(ModificationName)) ;
	// trncpy( ModificationName, Dummy[11 + 4 * NumberOfSampleInformations + i], ) ;
	ModificationName[0]             = Dummy[11 + 4 * NumberOfSampleInformations][0] ;
	ModificationName[1]             = Dummy[11 + 4 * NumberOfSampleInformations][1] ;
	ModificationName[2]             = Dummy[11 + 4 * NumberOfSampleInformations][2] ;
